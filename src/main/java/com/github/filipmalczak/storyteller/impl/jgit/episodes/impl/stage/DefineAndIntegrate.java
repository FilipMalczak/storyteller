...
public class DefineAndIntegrate implements Stage {
    ...
    private class Progress {
        boolean shouldCommitDefine;
        boolean shouldTagDefine;
        boolean shouldBranch;
        boolean shouldIntegrate;
        String scopeBranch;
        ...
        public void init(){
            shouldCommitDefine = true;
            shouldTagDefine = true;
            shouldBranch = true;
        ...
        public void plan(){
            ...
            //todo
            var pullResult = workingCopy.getRepository().pull().setRemoteBranchName(scopeBranch).call();
            require(pullResult.isSuccessful(), "pulling must be succesful");
            ...
            if (scopeIndex.size() > episodeInScopeIdx) { // definition for this episode already present
                ...
                log.atInfo().log("Episode %s already defined ", definition);
                require(
                    toDefine,
...
//                "If the episode has already been defined, then definitions must match"
            ...
            subjectBranch = buildRefName(id, PROGRESS);
            log.atFine().log("Subject branch name resolved to %s", subjectBranch);
            if (workingCopy.branchExists(subjectBranch)){
                shouldBranch = false;
                log.atInfo().log("Branch %s already present", subjectBranch);
                //todo safeguard that it was forked from current progress
            } else {
                log.atInfo().log("Branch %s missing", subjectBranch);
            }
        ...
        public void exec(){
            ...
            if (shouldCommitDefine){
                ...
                doCommitDefine(id, scopeBranch, defineName);
            ...
            if (shouldTagDefine){
                ...
                doTagDefine(scopeBranch, defineName);
                log.atInfo().log("Creating the tag finished");
            } else {
                log.atInfo().log("Tag %s already present", defineName);
            }
            if (shouldBranch){
                log.atInfo().log("Creating branch %s", subjectBranch);
                doBranch(subjectBranch);
            ...
            if (shouldIntegrate){
                log.atInfo().log("Integrating subepisode %s to branch %s", toDefine, subjectBranch);
                doCommitAndTagIntegrate(scopeBranch, subjectBranch, buildRefName(id, END), integrateName);
                log.atInfo().log("Integration finished");
            } else {
                log.atInfo().log("Integration of subepisode %s to branch %s has already happened", toDefine, subjectBranch);
            }
        }
